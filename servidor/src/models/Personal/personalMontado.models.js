...
export async function obtenerPersonalMontado() {
    try {
        ...
        const result = await pool.request().query(
            ...
            WHERE CAST(l.registro AS date) = CAST(GETDATE() AS date) 
                ...
                AND e.departamento IN ('PROD ML6', 'ENTRADA DEL TURNO FALTA', 'ENTRADA DEL TURNO RETARDO')`);

        return result.recordset;

    } catch (error) {
        console.error('Error al obtener personal montado:', error);
        throw error;
    }
}