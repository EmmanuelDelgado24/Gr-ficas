...
export function iniciarVerificacionInyeccion(io) {
  ...
  const verificar = async () => {
    try {
      ...
      if (datosPrevios4DigitosSumaPares !== sumaPares4DigitosActual) {
        ...
        io.emit(`actualizar-inyeccion`, lotes4Digitos);
      ...
      if (datosPrevios5DigitosSumaPares !== sumaPares5DigitosActual) {
        ...
        io.emit(`actualizar-inyeccion-5digitos`, lotes5Digitos);
        console.log(`[SocketIO] Pares cambiaron en inyeccion 5 Dígitos: ${sumaPares5DigitosActual}`);
      } else {
        console.log(`[SocketIO] Pares de inyeccion 5 Dígitos sin cambios: ${sumaPares5DigitosActual}`);
      }
    } catch (error) {
      console.error(`Error al verificar inyeccion:`, error);
    } finally {
