...
export function iniciarVerificacionSuela(io) {
  ...
  const verificar = async () => {
    try {
      ...
      if (datosPrevios4DigitosSumaPares !== sumaPares4DigitosActual) {
        ...
        io.emit(`actualizar-suela`, lotes4Digitos);
        console.log(`[SocketIO] Pares cambiaron en  4 Dígitos: ${sumaPares4DigitosActual}`);
      } else {
        console.log(`[SocketIO] Pares de inyección 4 Dígitos sin cambios: ${sumaPares4DigitosActual}`);
      }
      ...
      if (datosPrevios5DigitosSumaPares !== sumaPares5DigitosActual) {
        ...
        io.emit(`actualizar-suela-5digitos`, lotes5Digitos);
        console.log(`[SocketIO] Pares cambiaron en suela 5 Dígitos: ${sumaPares5DigitosActual}`);
      } else {
        console.log(`[SocketIO] Pares de suela 5 Dígitos sin cambios: ${sumaPares5DigitosActual}`);
      }
    } catch (error) {
      console.error(`Error al verificar suela:`, error);
    } finally {
