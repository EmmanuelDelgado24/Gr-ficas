...
const AuditoriaL1 = () => {
  ...
  useEffect(() => {
    ...
    socket.on("connect", () => {
      console.log("Conectado al servidor de WebSockets L1 Auditoria");
      // console.log(socket.id);
    });

    // Escuchar evento específico
    socket.on("actualizar-4D-Auditoria-L1", (datos) => {
      console.log("Datos Actualizados en Auditoria");
    ...
    return () => {
      ...
      setLoading(false);
      socket.off("connect");
      socket.off("actualizar-4D-Auditoria-L1");
    };
  }, []);
