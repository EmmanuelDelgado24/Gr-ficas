...
export const TablaPespunteL6 = () => {
  const [data, setData] = useState(null);
  const [error, setError] = useState(null);

  useEffect(() => {
    setInterval(() => {
      fetch("https://api.avances-pazstor.online/avances/Pespunte245-4")
        .then((response) => {
          if (!response.ok) {
    ...
    //fetch("http://localhost:3000/avances/Pespunte241")
    //  .then((response) => {
    //    if (!response.ok) {
    //      throw new Error("Error en la respuesta del servidor");
    //    }
  ...
  return (
    <div>
      <h1 className="text-xl font-bold mb-4">PESPUNTE L-6  4 DIGITOS</h1>
...
export default TablaPespunteL6;