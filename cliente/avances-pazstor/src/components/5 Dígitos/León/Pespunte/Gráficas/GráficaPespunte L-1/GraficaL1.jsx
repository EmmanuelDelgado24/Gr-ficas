...
const GraficaL1 = () => {
  ...
  const conectarSocket = () => {
    if (!socket.connected) {
      console.log("Intentando conectar al socket 4L1...");
      socket.connect();
    }
  };
  ...
  // Intentar reconectar cada 5 segundos si aún no conecta
  const intervalo = setInterval(() => {
    if (!socket.connected) {
      console.warn("Reintentando conexión al socket 4L1...");
      conectarSocket();
    }
  }, 5000);

  socket.on("connect", () => {
    console.log("✅ Conectado al servidor de WebSockets L1");
  });

  socket.on("actualizar-Pespunte242", (datos) => {
    console.log("Datos actualizados en L1");
  ...
  return () => {
    ...
    socket.off("connect");
    socket.off("actualizar-Pespunte242");
      ...
      <div>
          <div className="max-w-lg p-6 border border-gray-100 rounded-lg shadow-sm bg-gray-800 border-dark-700">
            <div>
              ...
              <p className="text-2xl font-normal text-gray-400">
                Modelos: <br/>
                <span className={`font-bold ${modelos.length > 0 ? "text-white" : "text-lg text-yellow-400"}`}>
