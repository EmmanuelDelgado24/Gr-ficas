...
const GraficaL6 = () => {
  ...
  useEffect(() => {
    ...
    socket.on("connect", () => {
      console.log("Conectado al servidor de WebSockets Pespunte L6");
    });
        ...
        <div className="max-w-lg p-6 border border-gray-100 rounded-lg shadow-sm bg-gray-800 border-dark-700">
          <div>
            ...
            <p className="text-2xl font-normal text-gray-400">
              N° total pares:{" "}
              <span className="font-bold text-white"> {sumaLC_PARLOT} </span>
            </p>
            <p className="text-2xl font-normal text-gray-400">
              Modelos: <br/>
              <span
                className={`font-bold ${
                  modelos.length > 0 ? "text-white" : "text-yellow-400"
                }`}
              >
                {modelos.length > 0
                  ? modelos.join(", ")
